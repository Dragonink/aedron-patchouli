...
pub(super) fn Libraries<G: Html>(cx: Scope) -> View<G> {
	...
	let manage_library_props = ManageLibraryProps {
		dialog_ref,
		managed_library_sig,
		libraries,
	};
	...
	main()
		...
		.c(Suspense(cx, props))
		.c(ManageLibrary(cx, manage_library_props))
		.view(cx)
}
		...
		.on("close", move |_| {
			...
			sycamore::futures::spawn_local_scoped(cx, async move {
				match crate::send_api(&req).await.unwrap() {
					204 => libraries.modify().retain(|user| {
						user.ne(managed_library_sig.get().as_ref().as_ref().unwrap())
					}),
					_ => web_sys::window()
						.unwrap()
						.alert_with_message(
